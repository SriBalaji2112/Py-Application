...
def onSelecte(event):
    ...
    curItem = header.focus()
    contents = (header.item(curItem))
    ...
    UpdateWindow.title("Edit Contact")
    width = 400
    height = 300
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
    ...
    # ===================LABELS==============================
    lblTitle = Label(FormTitle, text="Updating Contacts", font=('arial', 16), bg="orange", width=300)
    ...
    # ==================BUTTONS==============================
    btn_updatecon = Button(ContactForm, text="Update", width=50, command=updateData, bg="orange")
    btn_updatecon.grid(row=6, columnspan=2, pady=10)


# fn1353p
def deleteData():
    if not header.selection():
        result = tkMessageBox.showwarning('Delete Contact', 'Please Select Something First!', icon="warning")
    else:
        result = tkMessageBox.askquestion('Delete Contact', 'Are you sure you want to delete this record?',
                                          icon="warning")
        if result == 'yes':
            curItem = header.focus()
            contents = (header.item(curItem))
            selecteditem = contents['values']
            header.delete(curItem)
            conn = sqlite3.connect("database.db")
            cursor = conn.cursor()
...
def addNewWin():
    ...
    # ===================LABELS==============================
    lblTitle = Label(FormTitle, text="Adding New Contacts", font=('arial', 16), bg="#66ff66", width=300)
    lblTitle.pack(fill=X)
    lbl_firstname = Label(ContactForm, text="Firstname", font=('arial', 14), bd=5)
    lbl_firstname.grid(row=0, sticky=W)
    lbl_lastname = Label(ContactForm, text="Lastname", font=('arial', 14), bd=5)
    ...
    # ==================BUTTONS==============================
    btn_addcon = Button(ContactForm, text="Save", width=50, command=submitData, bg="#66ff66")
...
Mid.pack(side=TOP)
midPointLeft = Frame(Mid, width=100)
midPointLeft.pack(side=LEFT, pady=10)
midPointLeftPadding = Frame(Mid, width=370)
midPointLeftPadding.pack(side=LEFT)
MidRight = Frame(Mid, width=100)
MidRight.pack(side=RIGHT, pady=10)
tableFrame = Frame(root, width=700)
tableFrame.pack(side=TOP)
# ============================LABELS======================================
lblTitle = Label(Top, text="Contact Management System", font=('arial', 16), width=500)
lblTitle.pack(fill=X)
...
header = ttk.headerview(tableFrame,
...
header.heading('MemberID', text="MemberID", anchor=W)
header.heading('Firstname', text="Firstname", anchor=W)
header.heading('Lastname', text="Lastname", anchor=W)
header.heading('Gender', text="Gender", anchor=W)
header.heading('Age', text="Age", anchor=W)
header.heading('Address', text="Address", anchor=W)
header.heading('Contact', text="Contact", anchor=W)
header.column('#0', stretch=NO, minwidth=0, width=0)
header.column('#1', stretch=NO, minwidth=0, width=0)
header.column('#2', stretch=NO, minwidth=0, width=80)
header.column('#3', stretch=NO, minwidth=0, width=120)
header.column('#4', stretch=NO, minwidth=0, width=90)
header.column('#5', stretch=NO, minwidth=0, width=80)
header.column('#6', stretch=NO, minwidth=0, width=120)
header.column('#7', stretch=NO, minwidth=0, width=120)
header.pack(pady=20)
header.bind('<Double-Button-1>', onSelecte)

# ============================BUTTONS=====================================
btn_add = Button(midPointLeft, text="+ ADD NEW", bg="#66ff66", command=addNewWin, width=20)
btn_add.pack()
btn_delete = Button(MidRight, text="DELETE", bg="orange", command=deleteData, width=20)
